...
const courses = [
  { id: 'Class 7 & 8', name: 'Class 7 & 8', price: 15000, color: 'from-fuchsia-500 to-indigo-600', icon: '🎓' },
  { id: 'Class 5 & 6', name: 'Class 5 & 6', price: 10000, color: 'from-sky-500 to-indigo-600', icon: '🌟' },
  { id: 'Class 3 & 4', name: 'Class 3 & 4', price: 8000, color: 'from-emerald-500 to-indigo-600', icon: '📚' },
  { id: 'Vedic Maths', name: 'Vedic Maths', price: 1500, color: 'from-amber-500 to-orange-600', icon: '🧮' },
  { id: 'Abacus', name: 'Abacus Long Term', price: 6000, color: 'from-rose-500 to-pink-600', icon: '🧠' },
  { id: 'Phonics', name: 'Phonics (Kids)', price: 10000, color: 'from-violet-500 to-purple-600', icon: '🗣️' },
  { id: 'Telugu', name: 'Telugu (Basics & Adv)', price: 4000, color: 'from-cyan-500 to-blue-600', icon: '🗣️' },
  { id: 'Olympiad', name: 'Olympiad Prep', price: 12000, color: 'from-red-500 to-orange-600', icon: '🏆' }
];
...
export default function RegisterPage() {
  ...
  const handleNext = () => {
    ...
    if (step === 1 && (!formData.studentName || !formData.parentName || !formData.studentClass || !formData.email || !formData.phone)) return;
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        setRegistrationError('This email is already registered. Please use a different email or sign in.');
      } else if (error.code === 'auth/operation-not-allowed') {
        setRegistrationError('Registration is not yet enabled in the Firebase Console. Go to Authentication > Sign-in method and Enable "Email/Password".');
      } else {
  ...
  const handleLocalBypass = async () => {
    ...
    localStudents.push({ ...formData, id: mockUid, password: DEFAULT_PASSWORD });
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-[#f8fafc] flex items-center justify-center p-4 py-20 relative font-sans">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-purple-600/10 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '2s' }}></div>
          ...
          <div className="bg-white/[0.02] border-b border-white/5 p-6 pb-0">
             <div className="flex justify-between items-center mb-6 px-4">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 mb-1">Step {step} of 3</span>
                  <h1 className="text-xl font-bold tracking-tight">
                    {step === 1 ? 'Personal Profile' : step === 2 ? 'Academy Choice' : 'Finalize & Enroll'}
                  </h1>
                </div>
        ...
        <div className="p-8">
          <AnimatePresence mode="wait">
            ...
            {step === 1 && !isSuccess && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Parent/Guardian Name</label>
                    <div className="group relative">
                      <Globe className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-500 transition-colors" size={18} />
                      <input 
                        type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Current Grade / Class</label>
                    <div className="group relative">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Registration Email (Login credentials will be sent here)</label>
                  <div className="group relative">
                ...
                <div className="pt-4 flex justify-between items-center bg-white/[0.02] p-4 rounded-2xl border border-white/5">
                  <div className="flex flex-col">
                    <span className="text-xs font-medium text-gray-400">Total Steps</span>
                    <span className="text-sm font-bold text-white">01 / 03</span>
                  </div>
                  <button 
                    onClick={handleNext}
            ...
            {step === 2 && !isSuccess && (
              ...
              >
                <div>
                  <label className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-4 block ml-1 text-center">Premium Educational Programs</label>
                    ...
                    {courses.map((course) => (
                      ...
                      >
                        <div className="flex flex-col gap-2 relative z-10">
                          ...
                          <span className="text-xs font-bold text-white tracking-tight">{course.name}</span>
                          <span className="text-[10px] text-gray-400">Premium Curricula</span>
                        </div>
                        <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${course.color} opacity-0 group-hover:opacity-10 transition-opacity blur-2xl`}></div>
                        {formData.course === course.id && (
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 text-center block">Optimal Timing Integration</label>
            ...
            {step === 3 && !isSuccess && (
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-1">Review your selections before processing</p>
                </div>

                <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-4">
                   <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Enrolled Program</span>
                        <div className="flex items-center gap-2">
                           <span className="text-xl">{selectedCourse?.icon}</span>
                           <p className="text-sm font-bold text-white">{selectedCourse?.name}</p>
                        </div>
                      </div>
                      <div className="bg-blue-600/10 px-3 py-1 rounded-full border border-blue-600/20">
                         <span className="text-[10px] font-bold text-blue-500 uppercase">Live Session</span>
                      </div>
                   </div>
                   ...
                   <div className="flex justify-between items-center py-4 border-y border-white/5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Selected Window</span>
                      </div>
                      <span className="text-xs font-bold text-gray-200">{formData.slot}</span>
                   </div>

                   <div className="flex justify-between items-center pt-2">
                      <span className="text-sm font-bold text-white">Investment Amount</span>
                      <div className="text-right">
                        <span className="text-2xl font-black text-blue-500 tracking-tighter">₹{selectedCourse?.price}</span>
                        <p className="text-[8px] text-gray-500 uppercase font-black tracking-widest">Includes all taxes</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="p-4 bg-orange-600/5 border border-orange-600/10 rounded-2xl flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-[10px] font-bold text-white uppercase tracking-widest">Security Protocol</h4>
                    <p className="text-[9px] text-gray-500">This is a secure sandbox environment. No actual funds will be transferred.</p>
                  </div>
                </div>
                ...
                {registrationError && (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <p className="text-[10px] text-red-200 font-medium leading-relaxed">{registrationError}</p>
                    </div>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  >
                    <span className={isProcessing ? 'opacity-0' : 'opacity-100 flex items-center justify-center gap-2'}>
            ...
            {isSuccess && (
              ...
              >
                ...
                <div className="space-y-2">
                  <h1 className="text-3xl font-black text-white tracking-tighter">Verified Enrollment</h1>
                  <p className="text-xs text-gray-400 font-medium">Your global educational identity is ready</p>
                </div>

                <div className="bg-white/[0.03] p-8 rounded-3xl border border-white/5 text-left space-y-6 relative overflow-hidden group">
                  <Sparkles size={64} className="absolute -bottom-8 -right-8 opacity-5 text-white group-hover:opacity-10 transition-opacity" />
                  
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] font-black text-blue-500 uppercase tracking-[0.2em]">Authentication ID</label>
                    <p className="text-md font-bold text-white">{formData.email}</p>
                  </div>

                  <div className="flex justify-between items-center">
                    <div className="flex flex-col gap-1">
                      <label className="text-[9px] font-black text-blue-500 uppercase tracking-[0.2em]">Access Credentials</label>
                      <p className="text-md font-bold text-emerald-500 tracking-widest">Brilliant@123</p>
                    </div>
                ...
                <div className="space-y-4">
                   <p className="text-[10px] text-gray-500 font-medium max-w-xs mx-auto leading-relaxed">
                     Digital welcome kit and authentication certificates have been synchronized to your profile.
                   </p>
