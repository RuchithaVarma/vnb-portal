...
export default function Analytics() {
  ...
  const coursePerformance = [
    { name: 'Mathematics Advanced', students: 1234, completion: 85, revenue: 234567, rating: 4.8 },
    { name: 'Physics Fundamentals', students: 987, completion: 78, revenue: 198765, rating: 4.6 },
    { name: 'Chemistry Basics', students: 756, completion: 92, revenue: 154321, rating: 4.7 },
    { name: 'Biology Masterclass', students: 543, completion: 88, revenue: 123456, rating: 4.9 },
    { name: 'English Literature', students: 432, completion: 76, revenue: 98765, rating: 4.4 }
  ];

  const topPerformers = {
    teachers: [
      { name: 'Dr. Rajesh Kumar', students: 1234, revenue: 234567, rating: 4.9 },
      { name: 'Prof. Sarah Williams', students: 987, revenue: 198765, rating: 4.8 },
  ...
  const engagementMetrics = [
    ...
    { metric: 'Avg. Session Duration', value: '45 min', change: -5.2, trend: 'down' },
    { metric: 'Course Completion Rate', value: '78.5%', change: 8.3, trend: 'up' },
    { metric: 'Video Watch Time', value: '12,345 hrs', change: 15.7, trend: 'up' }
  ];
    ...
    <AdminLayout title="Analytics Dashboard">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">Track your platform performance and insights</p>
        </div>
          ...
          return (
            <div key={key} className="bg-white rounded-xl shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`flex items-center text-sm font-medium ${metric.color}`}>
                  {metric.growth > 0 ? '+' : ''}{metric.growth}%
                  {metric.growth > 0 ? <ArrowUp className="w-4 h-4 ml-1" /> : <ArrowDown className="w-4 h-4 ml-1" />}
                </span>
              </div>
              <h3 className="text-2xl font-bold">
                {key === 'revenue' ? `₹${metric.current.toLocaleString()}` : 
                 key === 'engagement' ? `${metric.current}%` : 
                 metric.current.toLocaleString()}
              </h3>
              <p className="text-gray-500 text-sm mt-1 capitalize">{key}</p>
              <p className="text-gray-400 text-xs mt-2">
                from {key === 'revenue' ? `₹${metric.previous.toLocaleString()}` : 
                     key === 'engagement' ? `${metric.previous}%` : 
                     metric.previous.toLocaleString()} last period
              </p>
            </div>
          })}
      </div>
              ...
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center">
                  <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mr-4">
                    ...
                    {index === 3 && <PlayCircle className="w-5 h-5 text-blue-600" />}
                  </div>
                  <div>
                ...
                <div className="flex items-center">
                  <span className="text-lg font-semibold mr-3">{item.value}</span>
              ...
              <div className="space-y-2">
                {topPerformers.teachers.map((teacher, index) => (
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium">₹{(teacher.revenue / 1000).toFixed(0)}K</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="space-y-3">
          {[
            { user: 'John Doe', action: 'Enrolled in Mathematics Advanced', time: '2 min ago', type: 'enrollment' },
            { user: 'Jane Smith', action: 'Completed Physics Quiz', time: '5 min ago', type: 'completion' },
            { user: 'Mike Johnson', action: 'Started Chemistry Basics', time: '8 min ago', type: 'start' },
            { user: 'Sarah Williams', action: 'Rated Biology Masterclass', time: '12 min ago', type: 'rating' }
          ].map((activity, index) => (
            <div key={index} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg">
              <div className="flex items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 ${
                  ...
                  activity.type === 'start' ? 'bg-yellow-100' :
                  'bg-purple-100'
                }`}>
                  ...
                  {activity.type === 'start' && <PlayCircle className="w-4 h-4 text-yellow-600" />}
